...
function initFormTramiteExterno() {
    ...
    cargarflujo($('#expedientes_pendientes'), 1, 9, 'PENDIENTE');
}
 ...
 /*   $.ajaxCall(urlApp + '/UsuarioController/listObjectUsuarioBE.htm', {poUsuarioBE: UsuarioBE}, false, function (response) {
        ...
        if (response.length > 0) {*/
            ...
            $.ajaxCall(urlApp + '/FlujoController/listarJQRegistrosFlujoBE.htm', {poFlujoBE: FlujoBE}, false, function (response) {
              ...
              /*  if (ind_pintar_grilla) {
                    PintarRowGrilla('grid_tramiteExterno', 'isleido', 'NO', null, '#136641');
                    PintarRowGrilla('grid_tramiteExterno', 'isleido', 'SI', null, 'blue');
                    PintarRowGrillaavanzado('grid_tramiteExterno', 'diasrestantes', '4', '>', '#BBFFDD', '#000');
                    PintarRowGrillaavanzado('grid_tramiteExterno', 'diasrestantes', '4', '<', '#FFD9C6', '#000');
                    PintarRowGrillaavanzado('grid_tramiteExterno', 'diasrestantes', '0', '<', '#FFBFBF', '#000');
                }*/
...
function cargarflujo(li, idestadoflujo, idusuario, estado) {
    ...
    if (idestadoflujo == 2 || idestadoflujo == 3 || idestadoflujo == 4) {
        ...
        jQuery("#grid_tramiteExterno").setGridParam().hideCol("semaforo").trigger("reloadGrid");
        ind_pintar_grilla = false;
        ...
        jQuery("#grid_tramiteExterno").setGridParam().showCol("semaforo").trigger("reloadGrid");
        ind_pintar_grilla = true;
    }
    cargarGrilla_TramiteExterno();
}
...
function generartraminterno(idexpediente, codigoexpediente) {
    ...
    a.href = urlApp + '/tramite-interno/mensaje?idexpediente=' + idexpediente+'&codigoexpediente='+codigoexpediente;
    //a.target = "_blank";
